...
async function carregarEAgruparTodasAsPaginas() {
    ...
    if (typeof ordenarProcessos === 'function') {
        ordenarProcessos(); 
    }
}
    ...
    function ordenarProcessos() {
        ...
        const processosComData = processos.map(a => {
            const area = a.querySelector('div[name="areasituacao"]');
            let dataTexto = '';
            ...
            if (area) {
                ...
                // 1. Encontra o span com o texto 'em'
                const emSpan = Array.from(area.querySelectorAll('span')).find(s => s.textContent.trim() === 'em');

                if (emSpan && emSpan.nextElementSibling) {
                    // O próximo elemento após 'em' é geralmente a data (pode ser <span> ou <b>)
                    dataEl = emSpan.nextElementSibling;

                    // O próximo elemento depois da data é a hora, mas precisamos garantir que é um span ou div
                    let nextEl = dataEl.nextElementSibling;

                    // Loop para encontrar o próximo SPAN, ignorando quaisquer <b> ou <br> entre
                    while (nextEl && nextEl.tagName !== 'SPAN' && nextEl.tagName !== 'DIV') {
                        nextEl = nextEl.nextElementSibling;
                    }

                    // Se for um SPAN e estiver oculto (como o seu .hidden-xs), assumimos que é a hora
                    if (nextEl && (nextEl.tagName === 'SPAN' || nextEl.tagName === 'DIV')) {
                        horaEl = nextEl;
                    }
                }

                if (dataEl) {
                    // Limpa o texto da data, removendo HTML interno se houver
                    dataTexto = dataEl.textContent.trim();

                    if (horaEl) {
                        // Concatena a hora se ela existir
                        dataTexto += ` ${horaEl.textContent.trim()}`;
                    }
                }
            }

            // Retorna o objeto para ser ordenado
            return {
                elemento: a,
                data: parseDate(dataTexto)
            };
        });

        // Ordena com base no estado de 'ordemAtual'
        processosComData.sort((a, b) => {
            if (isNaN(a.data) || isNaN(b.data)) return 0;
    ...
    /*
     // 🔹 NOVO: bloco do PRAZO
            ...
            */
            ...
            <i class='fa fa-columns fa-5x'></i><br>
...
chrome.runtime.onMessage.addListener((message) => {
    if (message.action === 'settingsUpdated') {
        chrome.storage.local.get('userSettings', (data) => {
            ...
            // Opcional: recarregar as tags na tela para aplicar a nova posição imediatamente
            location.reload(); 
        });
    }
});
 ...
 function applyTagToPage(tagInstanceId, data) {
    ...
    // Pega as configurações salvas ou usa o padrão
    const tagPosition = currentSettings.tagPosition || 'classic';
    const tagPositionV = currentSettings.tagPositionV || 'top';
    const tagPositionH = currentSettings.tagPositionH || 'end';

    
    const idBase = tagInstanceId.split('-')[0];

    if (!currentSettings.showTags) {
        const containers = document.querySelectorAll(`.custom-tag-container-list-${idBase}`);
        containers.forEach(c => c.remove());
        return;
    }

    const processoEl = document.querySelector(`a[data-processo="${idBase}"]`);
    if (!processoEl) return;

    // 1. Identifica o Alvo
    let targetParent;
    if (tagPosition === 'classic') {
        targetParent = processoEl.querySelector('div[name="areasituacao"]');
    } else {
        targetParent = processoEl;
    }

    if (!targetParent) return;

    // --- AJUSTE PARA NÃO PULAR LINHA ---
    // Forçamos o elemento pai (areasituacao ou o link) a aceitar itens lado a lado
    targetParent.style.display = 'flex';
    targetParent.style.flexWrap = 'wrap';
    targetParent.style.alignItems = 'center';
    targetParent.style.justifyContent = tagPositionH === 'start' ? 'flex-start' : 'flex-end';
    targetParent.style.gap = '4px'; 

    // 2. Gerenciar o Container de Tags
    let tagsContainer = targetParent.querySelector(`.custom-tag-container-list-${idBase}`);
    
    if (!tagsContainer) {
        tagsContainer = document.createElement('div');
        tagsContainer.className = `custom-tag-container-list-${idBase}`;
        
        // Estilo do container: inline-flex para fluir junto com os spans de texto
        ...
        // 3. MOVER A DIV DE SITUAÇÃO (ex: "Em Andamento")
        const divSituacao = processoEl.querySelector('.situacao-caixa-entrada');
        if (divSituacao) {
            divSituacao.style.margin = "0";
            divSituacao.style.display = "inline-flex";
            divSituacao.style.float = "none"; // Remove floats que quebram o flex
            tagsContainer.appendChild(divSituacao);
        }

        // Inserção no local correto
        if (tagPosition === 'classic' || tagPositionV === 'bottom') {
            targetParent.appendChild(tagsContainer);
        } else {
            targetParent.prepend(tagsContainer);
        }
    }

    // 4. Criar/Atualizar a Tag
    if (!data) {
    ...
    let colortext = isColorLight(data.color) ? "#3B3B3B" : "#fff";
    ...
    existing.style.cssText = `
        ...
        display: inline-flex;
        align-items: center;
        height: 20px; /* Altura fixa para alinhar com os spans de texto */
    `;
    
    existing.setAttribute('data-toggle', 'popover');
    existing.setAttribute('data-original-title', `${escapeHtml(data.name)}`);
    existing.innerHTML = `<span>${escapeHtml(data.name)}</span>`;
}
...
function injetarBotoesMoverLote() {
    ...
    btnExcluir.classList.remove('pull-right');
    btnExcluir.classList.add('pull-left');
    const menuEstatico = criarTemplateBotoes('grupo-ordenacao-estatico');
    menuEstatico.classList.add('pull-right');
